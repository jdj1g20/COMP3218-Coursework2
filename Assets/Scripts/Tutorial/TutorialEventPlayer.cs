...
using UnityEngine;
using TMPro;
public class TutorialEventPlayer : MonoBehaviour
{
    [SerializeField]
    TutorialAdvisorScript advisor;
    [SerializeField]
    GameObject eventCanvas, eventDescriptionBox, button1, button2;
    [SerializeField]
    TutorialTextRevealScript eventText;
    [SerializeField]
    ...
    public TutorialGameLoop mainGameLoopScript;
    ...
    public void PlayEvent(EventMain eventToPlay, bool first)
    {
        // Set up the currentEvent and currentEventEnded variables
        mainGameLoopScript.playingMainEvent = true;
        currentEvent = eventToPlay;
        currentEventEnded = false;
        Debug.Log("Starting event, first = " + first);
        // Reveal Advisor
        if (first) {
            RevealAdvisor();
        } else {
            StartCoroutine(StartInitialEventDescription());
        }
    ...
    public IEnumerator ButtonSelect1(int button) {
        ...
        Debug.Log("Next event: " + nextMainEvent);
        // Deactivate buttons
        button1.SetActive(false);
        button2.SetActive(false);
        // Wait a small amount
